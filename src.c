...
{
    src_writel(dev, MUnit.OMR,0);
    src_readl(dev, MUnit.OMR);
}
/*
 * aac_send_iop_reset: Issue an IOP reset
 *  @returns 1 if the status register changes after issuing iop reset
 *           0 if there is no change in status register
 *  
 *  
 * / 
*/
static int aac_send_iop_reset(struct aac_dev *dev, int bled)
{
	u32 var, reset_mask;
	...
	if((dev->supplement_adapter_info.SupportedOptions3 & 
		AAC_OPTION_SUPPORTED3_IOP_RESET_FIB_DUMP) && aac_fib_dump)
	{

		bled = aac_adapter_sync_cmd(dev, IOP_RESET_FW_FIB_DUMP,
					0, 0, 0, 0, 0, 0, NULL, NULL, NULL, NULL, NULL);
		// Sleep for 10 seconds to allow firmware to dump the FIBs
		msleep(10000);
	}

	aac_err(dev, "Issuing Controller Reset-%d\n", bled);
...
void aac_send_hardware_soft_reset(struct aac_dev *dev)
{
	u_int32_t val;
	...
	aac_err(dev, "HW SOFT reset has been issued\n");
...
static int aac_src_restart_adapter(struct aac_dev *dev, int bled, u8 reset_type)
{
	...
	if (bled)
		aac_err(dev,"Driver Init: Adapter kernel panic - %x\n", bled);

	/* When there is a BlinkLED, HW_IOP_RESET will not have an effect */
	if (bled >= 2) {
		if(dev->sa_firmware && reset_type & HW_IOP_RESET)
		{
			reset_type &= ~HW_IOP_RESET;
			// give additional time for the crash dump to be captured
			ssleep(90);
		}
	}
	...
	case IOP_HWSOFT_RESET:
		needs_hw_soft_reset = aac_send_iop_reset(dev, bled);
        	aac_err(dev, "After calling aac_send_iop_reset. needs_hw_soft_reset: %d\n",needs_hw_soft_reset);

		if (!needs_hw_soft_reset){
        		aac_err(dev, "IOP RESET succeeded\n");
			goto ret;
		}
		if (!dev->sa_firmware){
        		aac_err(dev,"IOP RESET failed for ARC FW\n");
			return -ENODEV;
		}
		
		aac_err(dev, "Issuing HW_SOFT reset...\n");
		aac_send_hardware_soft_reset(dev);
		dev->msi_enabled = 0;
	   	/*
	 	*  Smart Array FW needs up to 38 seconds to be ready. So, we wait 38 seconds
	 	*  to make sure FW is ready before doing re-init.
	 	*/	 
 		for (i=0;i<38;i++) ssleep(1);
		start = jiffies;
		do {
       			msleep(500);
			status = src_readl(dev, MUnit.OMR);
			...
			if (time_after(jiffies, start+HZ*SOFT_RESET_TIME)){
				aac_err(dev, "HW_SOFT reset already sent, return ENODEV here\n");
				return -ENODEV;
			}
	...
	case HW_IOP_RESET:
		...
		break;
	default:
		aac_err(dev,"Reset type not recognised:Reset-%u\n",reset_type);
	}
ret:
