...
static struct aac_srb * aac_scsi_common(struct fib * fib, struct scsi_cmnd * cmd)
{
	...
	struct aac_dev *dev = (struct aac_dev *)cmd->device->host->hostdata;
#endif
...
#ifdef AAC_SAS_TRANSPORT
	if(aac_transport_enabled(dev) && not_sa_raid_volume(dev,scmd_id(cmd))) {
		aac_hash_lookup_target(dev,scmd_id(cmd), &(srbcmd->channel),
				&(srbcmd->id));
	}
	else
#endif
...
#ifdef AAC_SAS_TRANSPORT
	if(aac_transport_enabled(dev) && not_sa_raid_volume(dev,scmd_id(cmd) )) {
		aac_hash_lookup_target(dev,scmd_id(cmd), &bus, &target);
	}
	else
#endif
	{
...
int aac_issue_bmic_identify(struct aac_dev* dev, u32 bus, u32 target)
{
    ...
    srbcmd = kzalloc(sizeof(struct aac_srb), GFP_KERNEL);

    if(identify_resp && srbcmd)
    {
        ...
        rcode = aac_bmic_cmd_send(dev, srbcmd, identify_resp, datasize);
		...
		//printk("%s: %d %d %d %d %d %llx\n",__FUNCTION__, identify_resp->phys_connector[1], identify_resp->phys_connector[0], identify_resp->phys_box_on_bus, identify_resp->phys_bay_in_box, identify_resp->phy_count, get_unaligned_be64(identify_resp->wwidl));
    }
    kfree(srbcmd);
    kfree(identify_resp);
    return rcode;
 ...
 */
int aac_bmic_cmd_send(struct aac_dev* dev,
            struct aac_srb *srbcmd,
		    void *resp_buf,
            int resp_len)
{
    ...
    lresp = (void *) aac_pci_alloc_consistent(dev->pdev, resp_len, &addr);
    if(!lresp) {
    ...
    memset(lresp, 0, resp_len);
    srb_ptr = fib_data(fibptr);
    memcpy(srb_ptr, srbcmd, sizeof(struct aac_srb));

    /* set the common request fields */
    srb_ptr->function       = cpu_to_le32(SRBF_ExecuteScsi);
    srb_ptr->timeout        = cpu_to_le32(10);
    srb_ptr->retry_limit    = 0;
    srb_ptr->cdb_size       = cpu_to_le32(12);
    srb_ptr->count          = cpu_to_le32(resp_len);

    sg64 = (struct sgmap64 *)&srb_ptr->sg;
    sg64->count = cpu_to_le32(1);
    sg64->sg[0].addr[1] = cpu_to_le32((u32)(((addr) >> 16) >> 16));
    sg64->sg[0].addr[0] = cpu_to_le32((u32)(addr & 0xffffffff));
    sg64->sg[0].count = cpu_to_le32(resp_len);
    ...
    /* copy the response */
    memcpy(resp_buf, lresp, resp_len);
bmic_error:
    aac_pci_free_consistent(dev->pdev, resp_len, lresp, addr);
fib_error:
    aac_fib_complete(fibptr);
    aac_fib_free(fibptr);
    return rcode;
 ...
 */
int aac_bmic_sense_subsystem_info(struct aac_dev* dev)
{
	struct aac_ciss_sense_subsystem_info *subsystem_resp;
    struct aac_srb  *srbcmd;
    int datasize;
    int rcode = -ENOMEM;
	u32 vbus, vid;
	...
	subsystem_resp = (struct aac_ciss_sense_subsystem_info *)
		kmalloc(datasize, GFP_KERNEL);
    srbcmd = kzalloc(sizeof(struct aac_srb), GFP_KERNEL);

    if(subsystem_resp && srbcmd)
    {
		vbus = (u32)le16_to_cpu(
			dev->supplement_adapter_info.VirtDeviceBus);
		vid = (u32)le16_to_cpu(
        ...
        rcode = aac_bmic_cmd_send(dev, srbcmd, subsystem_resp, datasize);
        if(likely(rcode >= 0)){
            dev->sas_address = get_unaligned_be64(subsystem_resp->port_wwn);
			dev->phy_count = subsystem_resp->internal_port_count + subsystem_resp->external_port_count;
#if 0
			printk("%s: sas_address=%llx internal=%d external=%d\n", __FUNCTION__, dev->sas_address, subsystem_resp->internal_port_count, subsystem_resp->external_port_count);

			printk("%llx-%d %llx-%d\n", get_unaligned_be64(subsystem_resp->add_int_port_info[0].sas_address), subsystem_resp->add_int_port_info[0].ConnID, get_unaligned_be64(subsystem_resp->add_int_port_info[1].sas_address), subsystem_resp->add_int_port_info[1].ConnID);
			printk("%llx-%d %llx-%d\n", get_unaligned_be64(subsystem_resp->add_int_port_info[2].sas_address), subsystem_resp->add_int_port_info[2].ConnID, get_unaligned_be64(subsystem_resp->add_int_port_info[3].sas_address), subsystem_resp->add_int_port_info[3].ConnID);
			printk("%llx-%d %llx-%d\n", get_unaligned_be64(subsystem_resp->add_int_port_info[4].sas_address), subsystem_resp->add_int_port_info[4].ConnID, get_unaligned_be64(subsystem_resp->add_int_port_info[5].sas_address), subsystem_resp->add_int_port_info[5].ConnID);
			printk("%llx-%d %llx-%d\n", get_unaligned_be64(subsystem_resp->add_int_port_info[5].sas_address), subsystem_resp->add_int_port_info[6].ConnID, get_unaligned_be64(subsystem_resp->add_int_port_info[7].sas_address), subsystem_resp->add_int_port_info[7].ConnID);

			printk("%llx-%d %llx-%d\n", get_unaligned_be64(subsystem_resp->add_ext_port_info[0].sas_address), subsystem_resp->add_ext_port_info[0].ConnID, get_unaligned_be64(subsystem_resp->add_ext_port_info[1].sas_address), subsystem_resp->add_ext_port_info[1].ConnID);
			printk("%llx-%d %llx-%d\n", get_unaligned_be64(subsystem_resp->add_ext_port_info[2].sas_address), subsystem_resp->add_ext_port_info[2].ConnID, get_unaligned_be64(subsystem_resp->add_ext_port_info[3].sas_address), subsystem_resp->add_ext_port_info[3].ConnID);
			printk("%llx-%d %llx-%d\n", get_unaligned_be64(subsystem_resp->add_ext_port_info[4].sas_address), subsystem_resp->add_ext_port_info[4].ConnID, get_unaligned_be64(subsystem_resp->add_ext_port_info[5].sas_address), subsystem_resp->add_ext_port_info[5].ConnID);
			printk("%llx-%d %llx-%d\n", get_unaligned_be64(subsystem_resp->add_ext_port_info[5].sas_address), subsystem_resp->add_ext_port_info[6].ConnID, get_unaligned_be64(subsystem_resp->add_ext_port_info[7].sas_address), subsystem_resp->add_ext_port_info[7].ConnID);
			
			printk("%llx %llx %llx %llx\n", subsystem_resp->internal_port_info[0].ParentPhyMap, subsystem_resp->internal_port_info[1].ParentPhyMap, subsystem_resp->internal_port_info[2].ParentPhyMap, subsystem_resp->internal_port_info[3].ParentPhyMap);
			printk("%llx %llx %llx %llx\n", subsystem_resp->internal_port_info[4].ParentPhyMap, subsystem_resp->internal_port_info[5].ParentPhyMap, subsystem_resp->internal_port_info[6].ParentPhyMap, subsystem_resp->internal_port_info[7].ParentPhyMap);
			printk("%llx %llx %llx %llx\n", subsystem_resp->external_port_info[0].ParentPhyMap, subsystem_resp->external_port_info[1].ParentPhyMap, subsystem_resp->external_port_info[2].ParentPhyMap, subsystem_resp->external_port_info[3].ParentPhyMap);
			printk("%llx %llx %llx %llx\n", subsystem_resp->external_port_info[4].ParentPhyMap, subsystem_resp->external_port_info[5].ParentPhyMap, subsystem_resp->external_port_info[6].ParentPhyMap, subsystem_resp->external_port_info[7].ParentPhyMap);
#endif
            adbg_init(dev, "sas_address=%llx\n",dev->sas_address);
        }
    }
    kfree(srbcmd);
	kfree(subsystem_resp);
	return rcode;
}
#endif
...
int aac_get_adapter_info(struct aac_dev* dev)
{
	...
	rcode = aac_bmic_sense_subsystem_info(dev);
	if(rcode < 0) {
	    aac_err(dev, "SAS address retrieval failed. code=%d\n", rcode);
	    return rcode;
	}
#endif
	...
	if (!dev->sync_mode && dev->sa_firmware &&
		...
		if (phys_luns && srbcmd) {
            ...
            rcode = aac_bmic_cmd_send(dev, srbcmd, phys_luns, datasize);

			/* analyse data */
			if (likely(rcode >= 0 && phys_luns->resp_flag == 2)) {
...
#ifdef AAC_SAS_TRANSPORT
        /* retreive the device SAS address */
        aac_sas_get_device_address(dev);
#endif		
	}
...
int aac_scsi_cmd(struct scsi_cmnd * scsicmd)
{
	...
	if(aac_transport_enabled(dev) && (cid >=  dev->maximum_num_containers)) {
                if(aac_hash_lookup_target(dev, scmd_id(scsicmd), &hba_bus_lookup,
				&hba_target_lookup))
		    		return FAILED;
...
#ifdef AAC_SAS_TRANSPORT
				if(aac_transport_enabled(dev)) {
		                        aac_hash_lookup_target(dev, scmd_id(scsicmd), &bus,
										&cid);
				}
				else
#endif
...
{
    struct aac_host_map *host_map_hash = NULL, *next_host_map_hash = NULL, *prev_host_map_hash = NULL;
    u32 key = 0;

    host_map_hash = kzalloc (sizeof(struct aac_host_map),GFP_KERNEL);
    if (host_map_hash){
	key = host_target%AAC_MAX_TARGETS;
	
        /* fill up the host map entry  */
        host_map_hash->hba_bus_num = hba_bus_num;
        host_map_hash->hba_target_num = hba_target_num;
        host_map_hash->next = NULL;

	/* insert it to host_map_hash array */
        next_host_map_hash = aac->host_map_hash[key];
	while(next_host_map_hash){
	    /* check if the entry exists  */
	    if ((hba_bus_num == next_host_map_hash->hba_bus_num) && (hba_target_num == next_host_map_hash->hba_target_num)){
                aac_err(aac, "conflicting host map entry\n");
                kfree(host_map_hash);
		return 1;
            }		
	    prev_host_map_hash = next_host_map_hash;
            next_host_map_hash = prev_host_map_hash->next;
	} 
        if (aac->host_map_hash[key] == NULL)
	    aac->host_map_hash[key] = host_map_hash;
        else{
	    host_map_hash->next = aac->host_map_hash[key];
            aac->host_map_hash[key] = host_map_hash;
	}
    }
    else{
        aac_err(aac,"Host map allocation failure\n");
        return 1;
    }
//    aac_err(aac,"Host map added: host target %d, hba_BT (%d : %d)\n",host_target,hba_bus_num,hba_target_num);
    return 0;
}

int aac_hash_remove_hostmap(struct aac_dev *aac,u32 host_target, u32 hba_bus_num, u32 hba_target_num )
{
    struct aac_host_map *next_host_map_hash = NULL, * prev_host_map_hash = NULL;
    u32 key = host_target%AAC_MAX_TARGETS;
    u8 found = 0;
    u8 step = 0;

    next_host_map_hash = aac->host_map_hash[key];
    prev_host_map_hash = aac->host_map_hash[key];
    /* go through the link list */
    while (next_host_map_hash){
	if((hba_bus_num == next_host_map_hash->hba_bus_num) && (hba_target_num == next_host_map_hash->hba_target_num) ){
	    	if (step == 0){
			/* Found it. First one in the list   */
			if (prev_host_map_hash->next == NULL ){
				/* Has only one in the list  */
				aac->host_map_hash[key] = NULL;
	    		}
			else{
				/* link the hash to the next one in the list  */
				aac->host_map_hash[key] = next_host_map_hash->next;
			}
	    }
	    else{
                prev_host_map_hash->next = next_host_map_hash->next;  
            }
            kfree(next_host_map_hash);
            found = 1;
            break;
        }
        prev_host_map_hash = next_host_map_hash;
    ...
    if (found){
//        aac_err(aac,"Host map removed: host target %d, hba_BT (%d : %d)\n",host_target,hba_bus_num,hba_target_num);
        return 0;
    }
    else
	return 1;
...
{
    struct aac_host_map *next_host_map_hash = NULL, *prev_host_map_hash;
    u32 hba_target_num = 0xFFFFFFFF;
    u32 hba_bus_num;
    u8 found = 0;
    u32 key = host_target_num % AAC_MAX_TARGETS;
              
    next_host_map_hash = aac->host_map_hash[key];
    while(next_host_map_hash){
	 hba_bus_num = next_host_map_hash->hba_bus_num;
         hba_target_num = next_host_map_hash->hba_target_num;
         if (aac->hba_map[hba_bus_num][hba_target_num].host_target_num == host_target_num){
             found = 1;
             break;
         }
         hba_target_num = 0xffffffff;
         prev_host_map_hash = next_host_map_hash;
         next_host_map_hash = prev_host_map_hash->next;
    }
	
    if (!found){
//	aac_err(aac,"Could not find a match of host_target_num %d. There is a problem!\n",host_target_num);
        return 1;
    }
    else{
        *hba_bus = hba_bus_num;   
        *hba_target = hba_target_num; 
